...
async def _upload_change_and_show(e):
    ...
    plot(my_image, "output_upload_pillow")


def plot(img, element_id):
    LOGGER.info(f'ploting image to {element_id}')
    fig = px.imshow(img)
    graphJSON = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
    js.Plotly.newPlot(element_id, js.JSON.parse(graphJSON))

def np2canvas(img, canvas_id):
